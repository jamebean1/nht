$PBExportHeader$w_category_details.srw
$PBExportComments$Window used to select multiple categories
forward
global type w_category_details from w_response_std
end type
type cb_rem_all from commandbutton within w_category_details
end type
type dw_category_details from u_dw_std within w_category_details
end type
type cb_cancel from commandbutton within w_category_details
end type
type cb_ok from commandbutton within w_category_details
end type
type cb_remove from commandbutton within w_category_details
end type
type cb_add from commandbutton within w_category_details
end type
type st_6 from statictext within w_category_details
end type
type ddlb_case_type from dropdownlistbox within w_category_details
end type
type st_5 from statictext within w_category_details
end type
type dw_category_org from u_outliner_std within w_category_details
end type
end forward
...
global w_category_details w_category_details

type variables
STRING i_sCategoryList[]
STRING i_cKeys[]
STRING i_cCaseType = 'I'
STRING i_cCaseTypeDesc 
...
i_cCaseType = w_case_detail_parms.i_cCaseType
...
i_dsCategoryList.dataobject = 'd_nv_category'
...
l_nUpper = UpperBound( w_case_detail_parms.i_sReportParms.category_array )
...
IF l_nUpper > 0 THEN
	...
	FOR l_nIndex = 1 TO l_nUpper
		l_nSearchRow = i_dsCategoryList.Find ( "category_id = '"+w_case_detail_parms.i_sReportParms.category_array[l_nIndex]+"'", 1, i_dsCategoryList.RowCount() ) 					
...
// Clear out the array
w_case_detail_parms.i_sReportParms.category_array = w_case_detail_parms.i_cResetArray
...
IF l_nDWRowCount > 0 THEN
	FOR l_nIndex = 1 TO l_nDwRowCount
			...
			w_case_detail_parms.i_sReportParms.category_array[l_nIndex] = &
					dw_category_details.GetItemString(l_nIndex,'category_id')


	NEXT
	
	w_case_detail_parms.i_bMultiCategory = TRUE
	
ELSE

		w_case_detail_parms.dw_category_org.fu_HLClearSelectedRows()
		w_case_detail_parms.i_bMultiCategory = FALSE
