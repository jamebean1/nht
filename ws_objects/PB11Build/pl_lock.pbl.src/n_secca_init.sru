$PBExportHeader$n_secca_init.sru
$PBExportComments$Application service to handle security initialization
forward
global type n_secca_init from datastore
end type
end forward

global type n_secca_init from datastore
string dataobject = "d_secca_init_std"
end type
global n_secca_init n_secca_init
...
//	l_Source = THIS.fu_GetSource( )
	...
	CHOOSE CASE l_Source
		CASE "R"
			...
			RegistryGet( Mid( l_RegistryKey, 1, ( Len( l_RegistryKey ) - 1 ) ), "devmode", RegString!, l_Info )
			IF Upper( l_Info ) = "ON" THEN
				l_AppVersion = "development"
				SECCA.MGR.i_ProgVersion = l_AppVersion
			END IF
...
on n_secca_init.create
call super::create
TriggerEvent( this, "constructor" )
end on

on n_secca_init.destroy
TriggerEvent( this, "destructor" )
call super::destroy
end on

event constructor;SECCA.MGR.i_ProgName      = fu_GetInitInfo(c_INIT_ProgramName)
SECCA.MGR.i_ProgVersion   = fu_GetInitInfo(c_INIT_ProgramVersion)
SECCA.MGR.i_ProgBMP       = fu_GetInitInfo(c_INIT_ProgramBMP)
SECCA.MGR.i_PlaqueDisplay = fu_GetInitInfo(c_INIT_PlaqueDisplay)
SECCA.MGR.i_PlaqueBMP     = fu_GetInitInfo(c_INIT_PlaqueBMP)
SECCA.MGR.i_ProgINI       = fu_GetInitInfo(c_INIT_INIFile)
SECCA.MGR.i_AdmUseLogin   = fu_GetInitInfo(c_INIT_AdmUseLogin)
end event
