...
$PBExportComments$Command button for adding a new record(s)
forward
global type u_cb_new from u_cb_main
end type
end forward
...
IF IsNull(i_ButtonDW) = FALSE THEN
   IF IsValid(i_ButtonDW) THEN
		i_ButtonDW.DYNAMIC Event pcd_New(1, 0)
	END IF
...
ELSEIF IsValid(FWCA.MGR.i_WindowCurrentDW) THEN
   ...
   FWCA.MGR.i_WindowCurrentDW.DYNAMIC Event pcd_New(1, 0)
