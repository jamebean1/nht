...
$PBExportComments$Window ancestor for main style windows and sheets
forward
global type w_main from w_root
end type
end forward
...
//  Date     Person     Description of Change
//  -------- ---------- --------------------------------------------
//  8/25/2000 K. Claver Added code to check validity of an object before
//								attempting to execute a function or event on
//								the object
//******************************************************************
//  Copyright ServerLogic 1994-1996.  All Rights Reserved.
//******************************************************************
...
IF i_NumContainers > 0 THEN
	FOR l_Idx = 1 TO i_NumContainers
		IF IsValid(i_Containers[l_Idx]) THEN
...
IF i_NumDataWindows > 0 THEN
	FOR l_Idx = 1 TO i_NumDataWindows
		IF IsValid(i_DataWindows[l_Idx]) THEN
...
//  Date     Person     Description of Change
//  -------- ---------- --------------------------------------------
//  8/25/2000 K. Claver Added code to check if object is valid before
//								executing function on the object
//******************************************************************
//  Copyright ServerLogic 1994-1996.  All Rights Reserved.
//******************************************************************
...
//  Date     Person     Description of Change
//  -------- ---------- --------------------------------------------
//  8/25/2000 K. Claver Added code to check the validity of an object
//								before attempting to execute a function or
//								event on the object.
//******************************************************************
//  Copyright ServerLogic 1994-1996.  All Rights Reserved.
//******************************************************************
...
FOR l_Idx = 1 TO i_NumContainers
	IF IsValid(container_name) THEN
...
//  Date     Person     Description of Change
//  -------- ---------- --------------------------------------------
//	 8/25/2000 K. Claver Added code to check validity of objects before
//								executing functions or events on them.
//******************************************************************
...
//  Date     Person     Description of Change
//  -------- ---------- --------------------------------------------
// 10/23/98  Beth Byers	Make sure WorkSpaceWidth and WorkSpaceHeight
//								are > 0 for calculations
//	8/25/2000 K. Claver  Added code to check validity of objects before
//								executing functions or events on them
//******************************************************************
//  Copyright ServerLogic 1992-1996.  All Rights Reserved.
...
BOOLEAN l_Control
INTEGER l_Idx, l_WorkSpaceWidth, l_WorkSpaceHeight
...
//------------------------------------------------------------------
//  If WorkSpaceWidth or WorkSpaceHeight = 0, set them to 1 to avoid
//  arithmetic errors.
//------------------------------------------------------------------
		l_WorkSpaceWidth = WorkSpaceWidth()
		l_WorkSpaceHeight = WorkSpaceHeight()
		
		IF l_WorkSpaceWidth = 0 then l_WorkSpaceWidth = 1
		IF l_WorkSpaceHeight = 0 then l_WorkSpaceHeight = 1
		
		i_WindowPrevState = WindowState
		l_NewScaleX   = l_WorkSpaceWidth  / i_WindowWidth
   	l_NewScaleY   = l_WorkSpaceHeight / i_WindowHeight
   	l_AspectRatio = i_WindowWidth / i_WindowHeight

   	IF l_WorkSpaceWidth/l_WorkSpaceHeight <= l_AspectRatio THEN
   	   l_TmpNewWidth  = l_WorkSpaceWidth
   	   l_TmpNewHeight = l_WorkSpaceWidth / l_AspectRatio
   	ELSE
   	   l_TmpNewHeight = l_WorkSpaceHeight
      	l_TmpNewWidth  = l_WorkSpaceHeight * l_AspectRatio
   	END IF
...
//  Date     Person     Description of Change
//  -------- ---------- --------------------------------------------
//  8/25/2000 K. Claver Added code to check validity of an object
//								before attempting to execute a function or
//							   event on the object.
//******************************************************************
//  Copyright ServerLogic 1992-1996.  All Rights Reserved.
...
IF i_NumWindowChildren > 0 THEN
	...
	FOR l_Idx = l_NumWindowChildren TO 1 STEP -1
		IF IsValid(i_WindowChildren[l_Idx]) THEN
...
STRING  l_FileLabels[], l_Label, l_NextLabel
BOOLEAN l_FirstDash 

//------------------------------------------------------------------
//  Set the current MDI frame.
//------------------------------------------------------------------

IF i_WindowSheet THEN
	FWCA.MGR.i_MDIFrame = ParentWindow()
END IF
...
IF i_ToolbarFromMDI THEN
	IF i_ToolbarPosition <> c_ToolbarNone THEN
		CHOOSE CASE i_ToolbarPosition
			CASE c_ToolBarTop
				FWCA.MGR.i_MDIFrame.ToolbarAlignment = AlignAtTop!
			CASE c_ToolBarBottom
				FWCA.MGR.i_MDIFrame.ToolbarAlignment = AlignAtBottom!
			CASE c_ToolBarLeft
				FWCA.MGR.i_MDIFrame.ToolbarAlignment = AlignAtLeft!
			CASE c_ToolBarRight
				FWCA.MGR.i_MDIFrame.ToolbarAlignment = AlignAtRight!
			CASE c_ToolBarFloat
				FWCA.MGR.i_MDIFrame.ToolbarAlignment = Floating!
		END CHOOSE
   	...
   	IF FWCA.MGR.i_ApplicationObject.ToolBarTips <> i_ToolBarTips THEN
		...
		END IF
		IF NOT FWCA.MGR.i_MDIFrame.ToolbarVisible THEN
			FWCA.MGR.i_MDIFrame.ToolbarVisible = TRUE
		END IF
	ELSE
		IF FWCA.MGR.i_MDIFrame.ToolbarVisible THEN
			FWCA.MGR.i_MDIFrame.ToolbarVisible = FALSE
		END IF
	END IF
ELSE
	IF i_ToolbarPosition <> c_ToolbarNone THEN
...
//------------------------------------------------------------------
//  If there are no PowerClass DataWindows and this window is a
//  sheet and the MDI has c_AutoConfigMenus set then set the File
//  menu items for the window.
//------------------------------------------------------------------

ELSE
	IF i_WindowSheet AND FWCA.MGR.i_MDIAutoConfigMenus AND i_NumContainers = 0 THEN
