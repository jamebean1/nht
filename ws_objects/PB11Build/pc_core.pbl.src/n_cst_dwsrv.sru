...
public function integer of_setrequestor (u_dw_std adw_requestor)
end prototypes
...
//		ll_rc = idw_requestor.event pfc_populatedddw (as_dddwname, ldwc_obj)
		//Since not using PFC to retrieve, set return code to 0 for no action
		ll_rc = 0
...
//				ll_rc = idw_Requestor.Event pfc_retrievedddw(ls_colname)
				//Since not using PFC for this, set return code to 0 for no action
				ll_rc = 0
				If ll_rc < 0 Then Return -1
				lb_dddwrefreshed = True
			ELSE 
				// DropDownDataWindow does not have arguments, refresh the data.
				If IsValid(idw_Requestor.i_DBCA) Then
					ll_rc = ldwc_obj.SetTransObject(idw_Requestor.i_DBCA) 
					If ll_rc < 0 Then Return -1					
...
//			ll_rc = idw_requestor.event pfc_populatedddw (ls_colname, ldwc_obj)
			//Since not using PFC here, set return code to 0 for no action
			ll_rc = 0
			if ll_rc < 0 then return -1
