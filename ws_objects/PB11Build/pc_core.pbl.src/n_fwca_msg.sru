$PBExportHeader$n_fwca_msg.sru
$PBExportComments$Application service to handle framework class messages
forward
global type n_fwca_msg from datastore
end type
end forward

global type n_fwca_msg from datastore
string DataObject="d_fwca_msg_std"
end type
global n_fwca_msg n_fwca_msg
...
INTEGER  l_Answer, l_MessageCnt, l_Row, l_StartPos
STRING   l_Text, l_ErrorStrings[]
STRING   l_Title, l_Before, l_After, l_Char
INTEGER  l_Icon
INTEGER  l_ButtonSet
INTEGER  l_ButtonDefault
INTEGER  l_Status
ICON     l_IconType
BUTTON   l_ButtonType

//------------------------------------------------------------------
//  See if we have a valid message ID.  Use the UPPER function in 
//  case the user passed in a lower case or mixed case id
//------------------------------------------------------------------
...
IF l_Title = "<application_name>" THEN
   l_Title = FWCA.MGR.i_ApplicationName
ELSE
...
IF IsValid(OBJCA.LOG) THEN
	IF OBJCA.LOG.i_CurrentLog <> "" AND num_strings > 0 THEN
		IF string_parms[1] = "PowerClass Error" THEN
...
on n_fwca_msg.create
call datastore::create
TriggerEvent( this, "constructor" )
end on

on n_fwca_msg.destroy
