$PBExportHeader$u_search_criticalcases.sru
forward
global type u_search_criticalcases from u_search
end type
type st_slider from u_slider_control_horizontal within u_search_criticalcases
end type
type tab_case_preview from uo_tab_case_preview within u_search_criticalcases
end type
type tab_case_preview from uo_tab_case_preview within u_search_criticalcases
end type
end forward
...
global u_search_criticalcases u_search_criticalcases
...
string is_source_type, is_case_type, is_priority, is_date_type, is_aging_date, is_from_date, is_to_date, is_dept, is_user
long il_line_of_business, il_appeal_type, il_service_type
end variables
...
public function integer of_retrieve (string as_source_type, string as_case_type, string as_priority, string as_date_type, string ad_aging_date, string ad_from_date, string ad_to_date, string as_dept, string as_user, long al_line_of_business, long al_appeal_type, long al_service_type)
end prototypes
...
public function integer of_retrieve (string as_source_type, string as_case_type, string as_priority, string as_date_type, string ad_aging_date, string ad_from_date, string ad_to_date, string as_dept, string as_user, long al_line_of_business, long al_appeal_type, long al_service_type);long ll_rows, ll_row, ll_casenumber
string ls_case_number

// Set these in case of a restored view
is_source_type = as_source_type
is_case_type = as_case_type
is_priority = as_priority
is_date_type = as_date_type
is_aging_date = ad_aging_date
is_from_date = ad_from_date
is_to_date = ad_to_date
is_dept = as_dept
is_user = as_user
il_line_of_business = al_line_of_business
il_appeal_type = al_appeal_type
il_service_type = al_service_type

ll_rows = dw_report.Retrieve(as_source_type, as_case_type, as_priority, as_date_type, ad_aging_date, ad_from_date, ad_to_date, as_dept, as_user, al_line_of_business, al_appeal_type, al_service_type)
ll_row = dw_report.GetRow()
...
on u_search_criticalcases.create
int iCurrent
call super::create
...
on u_search_criticalcases.destroy
call super::destroy
...
//		MessageBox (gs_appname, 'The configuration record for new appeal system was not found.')
		ls_using_new_appeals = 'N'
End Choose
// If not using the new appeals system, then use a dw without all of the appeals info so that it can be printed
IF ls_using_new_appeals = 'N' THEN
	dw_report.dataobject = 'd_criticalcases_noappeals'
END IF
...
//-----------------------------------------------------------------------------------------------------------------------------------
  SELECT cusfocus.reportconfig.rprtcnfgid  
   ...
   WHERE cusfocus.reportconfig.rprtcnfgnme = 'Search Critical Cases'   
           ;
...
type dw_report from u_search`dw_report within u_search_criticalcases
event ue_retrieve ( )
...
integer width = 2190
integer height = 780
string dataobject = "d_criticalcases"
end type
...
l_nRtn = of_Retrieve(is_source_type, is_case_type, is_priority, is_date_type, is_aging_date, is_from_date, is_to_date, is_dept, is_user, il_line_of_business, il_appeal_type, il_service_type)
...
ln_menu_dynamic.of_add_item('Print','print','', THIS)
...
type uo_titlebar from u_search`uo_titlebar within u_search_criticalcases
...
type st_separator from u_search`st_separator within u_search_criticalcases
end type
