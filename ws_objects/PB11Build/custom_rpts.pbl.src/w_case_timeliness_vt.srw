$PBExportHeader$w_case_timeliness_vt.srw
$PBExportComments$Vermont Parameter window for Case Timeliness report
forward
global type w_case_timeliness_vt from w_response_std
end type
type dw_parameters from datawindow within w_case_timeliness_vt
end type
type cb_cancel from u_cb_close within w_case_timeliness_vt
end type
type cb_ok from u_cb_close within w_case_timeliness_vt
end type
type st_1 from statictext within w_case_timeliness_vt
end type
end forward
...
integer width = 1518
integer height = 1204
string title = "Case Timeliness Report Parameters"
...
global w_case_timeliness_vt w_case_timeliness_vt
...
on w_case_timeliness_vt.create
int iCurrent
call super::create
...
on w_case_timeliness_vt.destroy
call super::destroy
...
integer taborder = 10
string title = "none"
string dataobject = "d_case_timeliness_parms_vt"
...
IF IsValid(FWCA.MGR.i_WindowCurrent) THEN
	...
	IF ISNULL(l_nNumToSample) OR STRING(l_nNumToSample) = '' OR l_nNumToSample <= 0 THEN
		messagebox(gs_AppName,"A value is required for 'Closed within 'n' days'.")
		dw_parameters.SetFocus()
