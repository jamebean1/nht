$PBExportHeader$u_dynamic_gui_manage_views_filters.sru
forward
global type u_dynamic_gui_manage_views_filters from u_dynamic_gui_manage_views
end type
end forward
...
global u_dynamic_gui_manage_views_filters u_dynamic_gui_manage_views_filters
...
If IsValid(ids_datastore) and ll_index <= ids_datastore.RowCount() Then
	...
	ls_findstring = '%FilterViewID=' + ls_RprtDfltId + '%'
	ls_notfindstring = '%FilterViewID=' + ls_RprtDfltId + '[0-9]%'
	...
	ls_findstring = '%FilterViewID[[]equals]' + ls_RprtDfltId + '%'
	ls_notfindstring = '%FilterViewID[[]equals]' + ls_RprtDfltId + '[0-9]%'
...
on u_dynamic_gui_manage_views_filters.create
call super::create
end on

on u_dynamic_gui_manage_views_filters.destroy
call super::destroy
end on
...
is_dataobjectname						= 'd_manage_view_filters' 
is_argument_to_publish				= 'CriteriaView'
end event

type cbx_default_view from u_dynamic_gui_manage_views`cbx_default_view within u_dynamic_gui_manage_views_filters
end type

type cbx_globallyavailable from u_dynamic_gui_manage_views`cbx_globallyavailable within u_dynamic_gui_manage_views_filters
end type

type lv_doctypes from u_dynamic_gui_manage_views`lv_doctypes within u_dynamic_gui_manage_views_filters
end type
